...
enum CONTROL_STATUS {RECORD, WAIT, QUIT} CONTROL_FLAG = WAIT;
bool to_record = false;//交互变量，true->准备记录一组数据
vector<double> ENCODER_DATA(5, 0);//编码器数据，用来计算TCP

int main(int argc, char * argv[]) try
{
    ros::init(argc, argv, "camera_calibration_opencv");
    ...
    while (waitKey(1) < 0 && getWindowProperty(window_name, WND_PROP_AUTOSIZE) >= 0 && !to_quit) {
        ...
        switch (CONTROL_FLAG) {
            case CONTROL_STATUS::RECORD : {
                CONTROL_FLAG = CONTROL_STATUS::WAIT;
                ...
                TCP_trasform << "Encoder data: " << ENCODER_DATA[0] << " " << ENCODER_DATA[1] << " " << ENCODER_DATA[2] << " " << ENCODER_DATA[3] << ENCODER_DATA[4] << endl;
                TCP_trasform << "Fixed X-Y-Z(x/y/z):  " << euler_angles[0] / PI * 180 << " " << euler_angles[1] / PI * 180 << " " << euler_angles[2] / PI * 180 << endl;
                TCP_trasform << "Position(mm):  " << T(0, 3) << " "<< T(1, 3) << " " << T(2, 3) << endl;
                TCP_trasform << "Transform:\n" << T << endl << endl;
                TCP_trasform.close();
                break;
            }
            case WAIT:
    ...
    //第五部分：矩阵格式转换
    ofstream T_gripper_cam("./T_gripper_cam.txt", ofstream::out);
...
void ros_thread() {//ROS线程，如果不另开线程的话,main里面会有两个while(一个是realsense的while， 一个是ros的spin)
    ...
    while (CONTROL_FLAG != CONTROL_STATUS::QUIT) {
        ros::spinOnce();
    }
}
