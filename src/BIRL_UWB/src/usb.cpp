...
const unsigned short USB::crc16_tab[32*8] = {
...
int USB::rcmIfInit(char* destAddr){
...
void USB::rcmIfClose(void)
{
    close(radioFd);
    isAvailable_ = false;
}

int USB::rcmIfGetPacket(void *pkt, unsigned maxSize)
{
    return rcmIfGetPacketSerial(pkt, maxSize);
}

int USB::rcmIfSendPacket(void *pkt, unsigned size)
{

    return rcmIfSendPacketSerial(pkt, size);
}

int USB::rcmIfGetPacketSerial(void *pkt, unsigned maxSize)
{
    int c=0, i, crc;
    unsigned short val;
...
int USB::rcmIfSendPacketSerial(void *pkt, unsigned size)
{
    int i;
    unsigned short val;
...
int USB::serTimedRead(void *buf, int cnt)
{
    fd_set fds;
    struct timeval tv;
    int total = 0, i;
...
int USB::serWrite(void *buf, int cnt)
{
    return write(radioFd, buf, cnt);
}

unsigned short USB::crc16(void *ptr, int len)
{
    int i;
